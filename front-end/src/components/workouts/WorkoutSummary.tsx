...
const WorkoutSummary = ({
  ...
  const exerciseCount =
    isEditing && editData
      ? editData.exercises.length
      : workout.exercises.length;
  const volume = workout.totalVolume.toLocaleString();
  const { shortLabel: visibilityLabel, icon: visibilityIcon } =
    getVisibilityDisplay(workout.visibility);
    ...
    <div className="bg-white rounded-kin-lg shadow-kin-medium p-4 sm:p-6 mb-6 space-y-3">
      ...
      {isEditing && editData ? (
        ...
        <div className="p-3 bg-kin-stone-50 rounded-kin-sm">
          <p className="text-xs font-semibold text-kin-teal font-inter mb-0.5">
            Notes
          </p>
...
const StatChip = ({
  ...
  <div className="flex flex-col leading-tight">
    <span className="text-base font-bold font-montserrat text-kin-navy">
      {value}
    </span>
