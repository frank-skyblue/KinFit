...
import { Visibility, VISIBILITY_OPTIONS } from "../../constants/options";
...
const NewWorkout = () => {
  ...
  useEffect(() => {
    const fetchExercises = async () => {
      ...
      try {
        ...
        setExercises((response as { exercises: Exercise[] }).exercises || []);
      } catch (error) {
  ...
  const handleAddExercise = (exercise: Exercise) => {
    const newExercise = createExerciseEntry(
      exercise,
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const oldIndex = workoutData.exercises.findIndex(
      (ex) => ex._dragId === active.id,
    );
    const newIndex = workoutData.exercises.findIndex(
      (ex) => ex._dragId === over.id,
    );
